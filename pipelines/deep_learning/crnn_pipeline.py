...
logger = get_logger("crnn_pipeline")

SEQUENCE_LENGTH = 32


class CRNNPipeline(TradingPipeline):
    ...
    def train(self, train_data, val_data=None):
        from models.crnn_model import train_crnn_model
        self.feature_cols = self._get_feature_cols(train_data)

        self.model, metrics = train_crnn_model(
            train_data, target_col='target', device=self.device
        )
        logger.info(f"CRNN train complete: {metrics}")
        return metrics
    ...
    def predict(self, data) -> PipelineOutput:
        ...
        return PipelineOutput(
            ...
            metadata={'model': 'crnn', 'sequence_length': self.sequence_length},
        )
