...
class LSTMPipeline(TradingPipeline):
    ...
    def train(self, train_data, val_data=None):
        ...
        # Delegate to existing train function
        self.model, metrics = train_lstm_model(
            train_data, target_col='target', device=self.device
        )
        logger.info(f"LSTM train complete: {metrics}")
        return metrics
    ...
    def predict(self, data) -> PipelineOutput:
        ...
        # Align index: sequences lose the first `sequence_length` rows
        ...
        return PipelineOutput(
            ...
            metadata={'model': 'lstm', 'sequence_length': self.sequence_length},
        )
