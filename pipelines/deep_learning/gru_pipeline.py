...
logger = get_logger("gru_pipeline")

SEQUENCE_LENGTH = 32


class GRUPipeline(TradingPipeline):
    ...
    def train(self, train_data, val_data=None):
        ...
        self.model, metrics = train_gru_model(
            train_data, target_col='target', device=self.device
        )
        logger.info(f"GRU train complete: {metrics}")
        return metrics
    ...
    def predict(self, data) -> PipelineOutput:
        ...
        return PipelineOutput(
            ...
            metadata={'model': 'gru', 'sequence_length': self.sequence_length},
        )
