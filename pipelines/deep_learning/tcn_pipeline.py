...
logger = get_logger("tcn_pipeline")

SEQUENCE_LENGTH = 32


class TCNPipeline(TradingPipeline):
    ...
    def train(self, train_data, val_data=None):
        from models.tcn_model import train_tcn_model
        self.feature_cols = self._get_feature_cols(train_data)

        self.model, metrics = train_tcn_model(
            train_data, target_col='target',
            sequence_length=self.sequence_length,
        )
        logger.info(f"TCN train complete: {metrics}")
        return metrics
    ...
    def predict(self, data) -> PipelineOutput:
        ...
        return PipelineOutput(
            ...
            metadata={'model': 'tcn', 'sequence_length': self.sequence_length},
        )
