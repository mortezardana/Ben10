...
logger = get_logger("transformer_pipeline")

SEQUENCE_LENGTH = 32


class TransformerPipeline(TradingPipeline):
    ...
    def train(self, train_data, val_data=None):
        ...
        self.model, metrics = train_transformer_model(
            train_data, target_col='target',
            sequence_length=self.sequence_length,
        )
        logger.info(f"Transformer train complete: {metrics}")
        return metrics
    ...
    def predict(self, data) -> PipelineOutput:
        ...
        return PipelineOutput(
            ...
            metadata={'model': 'transformer', 'sequence_length': self.sequence_length},
        )
