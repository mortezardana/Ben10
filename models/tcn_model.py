...
def train_tcn_model(df, target_col='target', sequence_length=32, test_size=0.2, val_size=0.1, epochs=50, batch_size=64):
    logger.info("Preparing data for TCN training")
    ...
    logger.info("Data split complete for TCN")

    model = Sequential()
    model.add(TCN(input_shape=(X_train.shape[1], X_train.shape[2]), return_sequences=False))
    ...
    logger.info("Evaluating TCN model")
    ...
    logger.info(f"TCN Test Accuracy: {acc:.4f}")
    return model, metrics


def predict_tcn(model, df, target_col='target', sequence_length=32):
    logger.info("Generating predictions with trained TCN model")
    df = df.dropna().reset_index(drop=True)
