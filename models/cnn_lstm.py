...
def train_cnn_lstm_model(df, target_col='target', sequence_length=32, test_size=0.2, val_size=0.1, epochs=50,
                         batch_size=64):
    logger.info("Preparing data for CNN-LSTM training")
    ...
    logger.info("Data split complete for CNN-LSTM")
...
def predict_cnn_lstm(model, df, target_col='target', sequence_length=32):
    logger.info("Generating predictions with trained CNN-LSTM model")
    df = df.dropna().reset_index(drop=True)
